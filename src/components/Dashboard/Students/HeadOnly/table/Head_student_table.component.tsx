...
import Head_student_table_row from "./Head_student_table_row.component";
import {
  table_content_style,
  table_header_style,
  table_parent_style,
...
const Head_student_table = (props: CurrentComponentProp) => {
        ...
        <tbody>
          {sortedStudents.map((student: Student, index: number) => (
            <React.Fragment key={student.primary_student_ID}>
              <Head_student_table_row student={student} index={index} />
            </React.Fragment>
          ))}
        </tbody>
      </table>
...
export default Head_student_table;
